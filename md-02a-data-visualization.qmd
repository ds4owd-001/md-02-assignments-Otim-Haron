...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    **Answer by Haron:**
    ...
    This is because geom_bar() function makes the height of the bars proportional to the number of observations of the dependent variable in the gapminder_2007 object.

    Since the geom_col() function used to obtain the above plot had the same dependent variable as the geom_bar() function and **also** it's independent variable (y) set to count; the two graphs had the same output.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    **Answer by Haron:**
    ...
    Like the geom_bar() function, geom_histogram() function makes the height of the bars proportional to the number of observations of the dependent variable in the gapminder_2007 object.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    **Answer by Haron:**
    ...
    The breaks argument sets tickmarks for the on the independent variable axis (lifeExp) and also modifies the general appearance of the graph to enhance visibility.

    When used in conjunction with the seq(40, 85, 2.5), the values on the x axis are adjusted to start from 40 and end at 85 with each bar block having an increment of 2.5.

    ```{r}
    ggplot(data = gapminder_2007,
           mapping = aes(x = lifeExp,
                         fill = continent)) + 
      geom_histogram(color = "grey30", breaks = seq(40, 85, 2.5))
    ```
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    **Answer by Haron:**
    ...
    country_colors in the console shows the different unique color codes assigned to particular countries.

    ?country_colors in the console pops up the help menu in the repository window detailing the general usage of the color schemes in the whole data set (gapminder) such as description, usage, and format.
